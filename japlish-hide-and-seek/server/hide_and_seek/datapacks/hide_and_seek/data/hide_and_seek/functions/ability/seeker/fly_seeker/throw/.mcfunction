# 雪玉を投げる
#snowballのタグ
data modify storage macro: throw_snowball.tag set value "fly_seeker_module"
...
data modify storage macro: throw_snowball.ride_contents set value 'item:{id:"minecraft:dirt",Count:1b,tag:{CustomModelData:1}}'
#関数を実行
function hide_and_seek:.util/throw_snowball/


#処理終了
tag @e[type=snowball,tag=fly_seeker_module,tag=not_yet,sort=nearest,limit=1] remove not_yet
...
execute at @s run playsound minecraft:entity.chicken.ambient master @a
