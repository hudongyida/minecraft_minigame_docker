...
$execute at @s run item replace entity @n[type=armor_stand,tag=item.utils.armor.armor_stand.update_storage.$(playeruuid)] weapon.mainhand from entity @s armor.chest
...
# 고정값 방어 스탯 설정
function it:tech/utils/armor/random_stat/get_additive {slot:chest}
...
# 비율 방어 스탯 설정
function it:tech/utils/armor/random_stat/get_multiplicative {slot:chest}
...
# 갑옷 업데이트
$execute at @s run item replace entity @s armor.chest from entity @n[type=armor_stand,tag=item.utils.armor.armor_stand.update_storage.$(playeruuid),distance=..0.5] weapon.mainhand
function it:tech/utils/armor/update_lore/chestplate/update
