...
$execute at @s run item replace entity @n[type=armor_stand,tag=item.utils.armor.armor_stand.update_storage.$(playeruuid),distance=..0.5] weapon.mainhand from entity @s armor.head
...
# 고정값 방어 스탯 설정
function it:tech/utils/armor/random_stat/get_additive {slot:head}
...
# 비율 방어 스탯 설정
function it:tech/utils/armor/random_stat/get_multiplicative {slot:head}
...
# 갑옷 업데이트
$execute at @s run item replace entity @s armor.head from entity @n[type=armor_stand,tag=item.utils.armor.armor_stand.update_storage.$(playeruuid),distance=..0.5] weapon.mainhand
function it:tech/utils/armor/update_lore/helmet/update
