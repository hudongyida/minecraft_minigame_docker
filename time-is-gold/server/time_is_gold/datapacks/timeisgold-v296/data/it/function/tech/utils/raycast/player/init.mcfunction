...
tag @s add item.utils.raycast.player.origin
$execute positioned ^ ^ ^ run summon marker ^ ^ ^ {Tags:["item.utils.raycast.player.marker", "item.utils.raycast.player.init_target.$(playerid)"]}
$execute positioned ^ ^ ^ as @s run data modify entity @n[type=marker,tag=item.utils.raycast.player.init_target.$(playerid),distance=..0.5] Rotation set from entity @s Rotation
...
$execute positioned ^ ^ ^ as @n[type=marker,tag=item.utils.raycast.player.init_target.$(playerid),distance=..0.5] run function it:tech/utils/raycast/player/set_data with storage timeisgold:function_api stack[-1]
tag @s remove item.utils.raycast.player.origin
