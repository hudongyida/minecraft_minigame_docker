...
$scoreboard players operation @s combat.modified_damage.apply += @a[scores={playerid=$(playerid)},limit=1] combat.player_attack.damage_deltaX10
$scoreboard players operation @s combat.modified_damage.apply *= @a[scores={playerid=$(playerid)},limit=1] combat.player_attack.damage_multiplier
...
function system:combat/modified_damage/handheld_swing/damage with storage timeisgold:function_api stack[-1]
