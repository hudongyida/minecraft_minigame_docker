...
$scoreboard players operation @s combat.modified_damage.apply += @a[scores={playerid=$(playerid)},limit=1] combat.magic.damage_deltaX10
$scoreboard players operation @s combat.modified_damage.apply *= @a[scores={playerid=$(playerid)},limit=1] combat.magic.damage_multiplier
...
function system:combat/modified_damage/magic/damage with storage timeisgold:function_api stack[-1]
