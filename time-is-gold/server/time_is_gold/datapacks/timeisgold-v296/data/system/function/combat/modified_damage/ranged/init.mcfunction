...
$scoreboard players operation @s combat.modified_damage.apply += @a[scores={playerid=$(playerid)},limit=1] combat.ranged.damage_deltaX10
$scoreboard players operation @s combat.modified_damage.apply *= @a[scores={playerid=$(playerid)},limit=1] combat.ranged.damage_multiplier
execute as @s run scoreboard players operation @s combat.modified_damage.apply /= #100 scoreboardConstant
...
function system:combat/modified_damage/ranged/damage with storage timeisgold:function_api stack[-1]
