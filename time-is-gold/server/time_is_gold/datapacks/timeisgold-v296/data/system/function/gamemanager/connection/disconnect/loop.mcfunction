...
function system:gamemanager/connection/disconnect/step with storage timeisgold:function_api stack[-1]

data remove storage timeisgold:function_api stack[-1]

scoreboard players add #SYSTEM system.gamemanager.connection.index 1
execute if score #SYSTEM system.gamemanager.connection.index <= #SYSTEM playerid run function system:gamemanager/connection/disconnect/loop