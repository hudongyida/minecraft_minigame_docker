...
execute as @a[team=bam,tag=playing,gamemode=adventure] at @s run title @s actionbar [{"text":"钻石: ","color":"dark_aqua"},{"text":"[","color":"aqua","bold":true},{"score":{"objective":"bam_diamond","name":"@s"},"color":"dark_aqua","bold":false},{"text":"/7","color":"dark_aqua","bold":false},{"text":"]","color":"aqua","bold":true}]
