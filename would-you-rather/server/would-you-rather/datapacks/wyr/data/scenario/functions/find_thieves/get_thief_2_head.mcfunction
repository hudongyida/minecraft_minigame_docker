...
data modify block 0 0 0 Items[{Slot:0b}].id set from entity @e[type=skeleton,tag=ft_thief_2,limit=1] ArmorItems[3].id
data modify block 0 0 0 Items[{Slot:0b}].tag set from entity @e[type=skeleton,tag=ft_thief_2,limit=1] ArmorItems[3].tag
setblock 32000 0 0 air
setblock 32000 0 0 oak_sign{Text1:'[{"selector":"@e[type=area_effect_cloud,tag=ft_thief_2_name,limit=1]","italic":false,"color":"yellow"}]',Text2:'',Text3:'',Text4:''} destroy
