...
execute as @a[tag=playing,team=hcm,gamemode=adventure] at @s run title @s actionbar [{"text":"目标: ","color":"yellow","bold":false},{"text":"[","color":"yellow","bold":false},{"score":{"objective":"hcm_points","name":"@s"},"color":"gold","bold":true},{"text":"/","color":"red","bold":false},{"text":"25","color":"gold","bold":true},{"text":"]","color":"yellow","bold":false}]
...
execute if score $time hcm_timer matches 0 run tellraw @a[tag=playing,team=hcm,gamemode=adventure] [{"text":"时间到了! 你没有猎杀足够多的怪物.","color":"red"}]
