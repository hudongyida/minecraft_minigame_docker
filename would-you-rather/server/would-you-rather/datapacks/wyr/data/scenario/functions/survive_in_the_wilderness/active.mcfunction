...
execute if score $time campfire_bar matches -1..0 run tellraw @a[tag=playing,team=sitw] {"text":"营火已经熄灭!","color":"gold"}
...
execute if score $time campfire_bar matches 1.. run title @a[team=sitw,nbt={SelectedItem:{id:"minecraft:stick"}},gamemode=adventure] subtitle {"text":"丢在篝火上以延长火焰时间!","color":"yellow"}
...
execute if score $time campfire_bar matches 1.. run title @a[team=sitw,nbt={SelectedItem:{id:"minecraft:mutton"}},gamemode=adventure] subtitle [{"text":"按","color":"red"},{"keybind":"key.use","color":"red"},{"text":"在篝火上做饭.","color":"red"}]

execute as @a[tag=playing,team=sitw,gamemode=adventure] at @s run title @s actionbar [{"text":"| 温暖 : ","color":"red"},{"score":{"objective":"sitw_heat","name":"@s"},"color":"red"},{"text":"/10","color":"red"},{"text":" | 饥饿: ","color":"gold"},{"score":{"objective":"sitw_d_hunger","name":"@s"},"color":"gold"},{"text":"/20","color":"gold"},{"text":" | 口渴: ","color":"blue"},{"score":{"objective":"sitw_thirst","name":"@s"},"color":"blue"},{"text":"/15","color":"blue"}]
...
title @a[team=sitw,scores={sitw_param=..1},gamemode=adventure] subtitle {"text":"生命机能失灵 . . .","color":"red"}
