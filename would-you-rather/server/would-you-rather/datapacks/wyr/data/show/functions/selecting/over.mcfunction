...
execute as @a[tag=playing,scores={scenario_sel=1}] at @s run tellraw @s [{"text":"你将尝试: \"","color":"blue"},{"selector":"@e[tag=blue_select,type=area_effect_cloud]"},{"text":"\""}]
execute as @a[tag=playing,scores={scenario_sel=2}] at @s run tellraw @s [{"text":"你将尝试: \"","color":"yellow"},{"selector":"@e[tag=yellow_select,type=area_effect_cloud]"},{"text":"\""}]
